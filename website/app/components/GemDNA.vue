...
const props = defineProps<{ dna: Record<string, unknown> | null }>()
...
const d = computed(() => props.dna as any)
...
<template>
  <div class="dna-panel" :class="{ loaded: !!dna }">
    ...
    <template v-if="d">
      ...
      <!-- Facets + tilt -->
      <div class="meta-row">
        <div class="meta-chip">{{ FACET_LABELS[d.facets as number] ?? `${d.facets}-gon` }}</div>
        <div class="meta-chip">Tilt {{ (d.tilt as number > 0 ? '+' : '') }}{{ Math.round((d.tilt as number) * 57.3) }}°</div>
      </div>
